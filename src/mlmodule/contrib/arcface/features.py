...
from mlmodule.torch.utils import l2_norm, torch_apply_state_to_partial_model
...
class ArcFaceFeatures(
    ...
    def get_default_pretrained_state_dict_from_provider(
        ...
        pretrained_state_dict = load_state_dict_from_url(
            download_url, file_name="model_ir_se50.pth", map_location=self.device
        )
    ...
    def bulk_inference(
        ...
        **options
        ...
        if remove_bad_quality_faces:
            ...
            norm_faces = (
                norm_faces @ np.ones((norm_faces.shape[1], 1)) / norm_faces.shape[1]
            )

            # Filter for faces with good quality
            good_faces = (
                1 - (features @ norm_faces)[:, 0]
            ) > ARCFACE_MEAN_DISTANCE_THRESHOLD
