...
def get_dataset(
    model: BaseTorchMLModule, input_type: str, input_files: List[str]
) -> TorchDataset:
    if input_type == "IM":
        shrink_input: Optional[Tuple[int, int]] = None
...
def run_fun(
    args: argparse.Namespace, metrics: Optional[MetricsCollector] = None
) -> None:
    ...
    with metrics.measure("time_run_module"):
        ...
        if hasattr(model, "load"):
            ...
            model.load()
        # Getting dataset
        dataset = get_dataset(model, args.input_type, args.input_files)
...
def _contrib_module(module_str):
    ...
    module = ".".join(elements[:-1])
    return getattr(import_module(f"mlmodule.contrib.{module}"), elements[-1])
...
def main():
    ...
    run.add_argument(
        "--batch-size", type=int, default=None, help="Batch size for inference"
    )
    run.add_argument(
    ...
    run.add_argument(
        "--extra-kwargs", metavar="KEY=VALUE", nargs="+", type=parse_key_value_arg
    )
    run.add_argument(
        "--metrics", action="store_true", help="Returns only metrics data in output"
    )
    run.add_argument(
        "--input-type",
        choices=("IM", "VI"),
