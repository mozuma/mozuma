...
from typing import IO, List, Optional, Tuple, TypeVar, Union
...
def load_path_or_url_file(img_path: Union[str, Path, IO]) -> Union[str, Path, IO]:
    if type(img_path) == str and (
        img_path.startswith("http://") or img_path.startswith("https://")
    ):
...
class BaseImageDataset(IndexedDataset[IndicesType, ImageDatasetType]):
    ...
    def __init__(
        ...
        image_path: List[str],
        to_rgb=True,
        shrink_img_size=None,
    ):
        super().__init__(indices, image_path)
        self.add_transforms(
            [
                LoadRGBPILImage(
                    shrink_image_size=shrink_img_size, mode="RGB" if to_rgb else None
                )
            ]
        )
        if to_rgb:
...
class ImageDataset(BaseImageDataset[str]):
    ...
    def __init__(self, image_path: List[str], to_rgb=True, shrink_img_size=None):
        super().__init__(
            image_path, image_path, to_rgb=to_rgb, shrink_img_size=shrink_img_size
        )
        self.transforms = [load_path_or_url_file] + self.transforms
