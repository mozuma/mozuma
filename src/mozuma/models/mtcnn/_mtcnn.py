...
import numpy as np
import torch
from facenet_pytorch.models.mtcnn import ONet, PNet, RNet, fixed_image_standardization
...
class MoZuMaMTCNN(torch.nn.Module):
    ...
    Note:
        This modifies the facenet MTCNN:

        - Adds a pretrained argument to the `__init__` method
        - Changed the returned type of `probabilities` in `detect` to be `None` instead of `[None]`
